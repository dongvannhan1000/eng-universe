...
import { updateVocab } from "../services/vocab.service";
import type { Vocab, CreateVocabInput } from "../types";
...
export function EditVocabDialog({ vocab, open, onOpenChange }: EditVocabDialogProps) {
  ...
  const [formData, setFormData] = useState<CreateVocabInput>({
    ...
    tags: vocab.tags,
    explanationEn: vocab.explanationEn || "",
    notes: vocab.notes || "",
  });
  ...
  // Reset form data when vocab changes or dialog opens
  useEffect(() => {
    if (open) {
      setFormData({
        word: vocab.word,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update the information for "{vocab.word}"</DialogDescription>
        </DialogHeader>
